...
def RunCmd(command:str):
    ...
    match cmd:
        ...
        case "delay":
            try:
                delay = int(GetValue(args[0]))
                print(f"Waiting {delay} seconds.")
                time.sleep(delay)
            except ValueError:
                print(f"Invalid delay value: {args[0]}")
        ...
        case "io":
            if(args[0] == "write"):
                try:
                    ...
                    print(f"Failed write io {args[1]}: {e}")
            ...
            if AreVariable(cmd):
                if args[0] == "set":
                    try:
                        ...
                        print(f"Failed to set variable {cmd}: {e}")
            else:
...
RunScript(str(systemScriptsPath) + "/test.jp")